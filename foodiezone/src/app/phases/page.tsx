...
const statusConfig = {
  ...
  partial: { label: 'IN PROGRESS', color: '#FF3D8C', bg: 'rgba(255,61,140,0.1)', border: 'rgba(255,61,140,0.25)' },
  pending: { label: 'PENDING', color: 'rgba(19,19,18,0.3)', bg: 'rgba(19,19,18,0.04)', border: 'rgba(19,19,18,0.1)' },
}
    ...
    <main style={{ background: '#FFF8EC', minHeight: '100vh' }}>
      ...
      <section className="pt-32 pb-16 px-5">
        <div className="max-w-5xl mx-auto">
          ...
          >
            FOODIEZONE<br />
            <span style={{ color: 'rgba(19,19,18,0.2)' }}>PHASES</span>
          </h1>
          <p className="font-body text-base max-w-lg" style={{ color: 'rgba(19,19,18,0.4)' }}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-10">
            {[
              ...
              { label: 'Phases Done Value', value: `R ${paidPrice.toLocaleString()}`, accent: '#FF3D8C' },
              { label: 'Full Project', value: `R ${totalPrice.toLocaleString()}`, accent: '#FF7A1A' },
            ].map((s) => (
              <div
                key={s.label}
                className="p-4"
                style={{ border: '1px solid rgba(19,19,18,0.07)', background: 'rgba(19,19,18,0.02)' }}
              >
                <div
                ...
                <div className="font-body text-xs mt-1 uppercase tracking-widest" style={{ color: 'rgba(19,19,18,0.3)' }}>
                  {s.label}
                </div>
              </div>
            ))}
          ...
          <div className="mt-8 max-w-sm">
            <div className="flex justify-between items-center mb-2">
              <span className="font-heading font-black text-xs uppercase tracking-widest" style={{ color: 'rgba(19,19,18,0.3)' }}>
                Overall Progress
              </span>
              <span className="font-heading font-black text-sm" style={{ color: '#FF7A1A' }}>
                {done + partial}/{phases.length} phases
              </span>
            </div>
            <div className="h-1.5 w-full" style={{ background: 'rgba(19,19,18,0.08)' }}>
              <div
                className="h-full"
                style={{
                  width: `${((done + partial * 0.5) / phases.length) * 100}%`,
                  background: 'linear-gradient(90deg, #FF7A1A, #FF3D8C)',
                }}
              />
            </div>
            <div className="flex gap-5 mt-3">
              {[
                ...
                { dot: '#FF3D8C', label: `${partial} In Progress` },
                { dot: 'rgba(19,19,18,0.2)', label: `${phases.length - done - partial} Pending` },
              ].map((s) => (
                <div key={s.label} className="flex items-center gap-1.5">
                  <div className="w-2 h-2 rounded-full" style={{ background: s.dot }} />
                  <span className="font-body text-xs" style={{ color: 'rgba(19,19,18,0.3)' }}>{s.label}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Phase cards */}
      <section className="pb-24 px-5" style={{ borderTop: '1px solid rgba(19,19,18,0.06)' }}>
          ...
          {phases.map((phase) => {
            ...
            return (
              <div
                ...
                style={{
                  background: phase.status === 'done' ? 'rgba(19,19,18,0.02)' : '#F5E9CC',
                  border: `1px solid ${phase.status === 'done' ? 'rgba(34,197,94,0.15)' : 'rgba(19,19,18,0.06)'}`,
                }}
              >
                ...
                <div
                  ...
                  style={{ fontSize: '8rem', color: 'rgba(19,19,18,0.02)', lineHeight: 1 }}
                >
                  {phase.number}
                </div>
                ...
                <div className="relative z-10 p-7 md:p-10">
                  ...
                  <div className="flex flex-wrap items-start gap-4 mb-2">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <span className="font-heading font-black text-xs shrink-0" style={{ color: 'rgba(19,19,18,0.2)' }}>
                        PHASE {phase.number}
                      </span>
                      <h2 className="font-heading font-black text-[#131312] uppercase text-xl md:text-2xl tracking-tight">
                        {phase.title}
                      </h2>
                    </div>
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <div
                        ...
                        style={{ color: phase.status === 'done' ? '#22c55e' : '#FF3D8C' }}
                      >
                        {phase.price}
                      </div>
                      {/* Status badge */}
                  ...
                  <p className="font-body text-sm mb-7" style={{ color: 'rgba(19,19,18,0.4)' }}>
                    {phase.description}
                  </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {/* Features */}
                    <div>
                      <h3 className="font-heading font-black text-xs uppercase tracking-[0.2em] mb-4" style={{ color: 'rgba(19,19,18,0.25)' }}>
                        Features Included
                      </h3>
                        ...
                        {phase.features.map((item) => (
                          <li key={item} className="flex items-center gap-2.5">
                            <div
                              ...
                              style={{
                                background: phase.status === 'done' ? 'rgba(34,197,94,0.15)' : 'rgba(19,19,18,0.04)',
                                border: `1px solid ${phase.status === 'done' ? 'rgba(34,197,94,0.4)' : 'rgba(19,19,18,0.1)'}`,
                              }}
                            >
                              {phase.status === 'done' && (
                              ...
                              {(phase.status === 'partial' || phase.status === 'pending') && (
                                <div className="w-1.5 h-1.5 rounded-full" style={{ background: phase.status === 'partial' ? '#FF3D8C' : 'rgba(19,19,18,0.2)' }} />
                              )}
                            </div>
                            <span className="font-body text-sm" style={{ color: phase.status === 'done' ? 'rgba(19,19,18,0.55)' : 'rgba(19,19,18,0.3)' }}>
                              {item}
                            </span>
                          </li>
                        ))}
                        ...
                        {phase.benefits.map((benefit) => (
                          <li key={benefit} className="flex items-start gap-2.5">
                            <div
                              ...
                              style={{ background: 'rgba(255,122,26,0.12)', border: '1px solid rgba(255,122,26,0.3)' }}
                            >
                              <div className="w-1.5 h-1.5 rounded-full" style={{ background: '#FF7A1A' }} />
                            </div>
                            <span className="font-body text-sm leading-snug" style={{ color: 'rgba(19,19,18,0.45)' }}>
                              {benefit}
                            </span>
                          </li>
                        ))}
                  ...
                  {(phase.route || phase.route2) && (
                    <div className="flex gap-3 mt-7 pt-6" style={{ borderTop: '1px solid rgba(19,19,18,0.06)' }}>
                      {phase.route && (
                        <Link
                          href={phase.route}
                      ...
                      {phase.route2 && (
                        <Link
                          href={phase.route2}
                          className="font-heading font-black text-xs uppercase tracking-widest px-4 py-2.5 transition-colors hover:text-[#131312]"
        ...
        <div
          ...
          style={{ border: '1px solid rgba(255,122,26,0.25)', background: 'rgba(255,122,26,0.05)' }}
        >
          <div className="flex flex-wrap items-center justify-between gap-6">
            <div>
              <p className="font-heading font-black text-xs uppercase tracking-[0.25em] mb-2" style={{ color: 'rgba(19,19,18,0.3)' }}>
                Full Project Investment
              </p>
              <div className="font-heading font-black text-[#131312] uppercase leading-none" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
                R {totalPrice.toLocaleString()}
              </div>
              <p className="font-body text-sm mt-2" style={{ color: 'rgba(19,19,18,0.35)' }}>
                All 5 phases · Full web app · No hidden costs
              </p>
            </div>
            <div className="space-y-2 text-right">
              <div className="font-body text-sm" style={{ color: 'rgba(19,19,18,0.3)' }}>
                Delivered value so far: <span className="font-heading font-black" style={{ color: '#22c55e' }}>R {paidPrice.toLocaleString()}</span>
              </div>
              <div className="font-body text-sm" style={{ color: 'rgba(19,19,18,0.3)' }}>
                Remaining phases: <span className="font-heading font-black" style={{ color: '#FF3D8C' }}>
      ...
      {/* Footer note */}
      <div className="text-center py-8 px-5" style={{ borderTop: '1px solid rgba(19,19,18,0.04)' }}>
        <p className="font-body text-xs mb-1" style={{ color: 'rgba(19,19,18,0.15)' }}>
